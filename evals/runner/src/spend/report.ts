...
import { MAX_RUN_COST_MICRO_USD } from "@novagait/agent";
...
export function renderMarkdown(
  ...
  push(
    ...
    "doubles again.",
    "",
    "**A6. No thinking tokens.** On Claude Opus 5 and Claude Sonnet 5,",
    "adaptive thinking is **on by default** when the parameter is omitted, and",
    "thinking tokens bill as output. Since LOT-119 the eval live lane sets",
    '`thinking: {type: "disabled"}` for matrix runs',
    "(`evals/runner/src/live-lane.ts`), and the agent loop only forwards it to",
    "models that accept it, so the Sonnet 5 and Opus 5 rows are estimated on",
    "the same no-thinking basis the matrix will actually run on. The",
    "production `claude-haiku-4-5` path sends no `thinking` field at all.",
    "",
    ...
    ...(meta.casesOverCap.length > 0
      ? [
          ...
          `[DEFAULT], currently ${meta.iterationCap}, so an Abhinav-approved`,
          "change), or rely on the live",
    ...
    ...(() => {
      ...
      const haiku = e.aggregates.find((a) => a.model === "claude-haiku-4-5");
      ...
      return [
        ...
        "",
        `| Model | Cost per interactive run | vs ${usd4(cap)} per-run cap |`,
        "|---|---:|---|",
        ...rows.map(
          (r) =>
        ...
        `and at ${usd4(rows[0].costUsd)} per run it sits above the`,
        `${usd4(cap)} \`MAX_RUN_COST_MICRO_USD\` breaker in`,
        ...
        "front of a buyer.",
        "",
        ...(haiku && !haiku.cacheApplies
          ? [
              "Prompt caching does not rescue it either: the cacheable prefix is",
              `${num(haiku.prefixTokens)} tokens against Haiku's`,
              `${num(PRICING["claude-haiku-4-5"].minCacheablePrefixTokens)}-token`,
              "minimum (see §3), so `cache_control` is silently ignored on exactly",
              "the model that needs it. Get the prefix over the minimum first: it",
              "is the only option that improves the demo rather than loosening a",
              "control, and the cached per-run cost falls toward the suffix-only",
              "figure.",
            ]
          : [
              "Note this row is the **uncached** cost. The cacheable prefix now",
              `measures ${num(haiku?.prefixTokens ?? 0)} tokens, over Haiku's`,
              `${num(PRICING["claude-haiku-4-5"].minCacheablePrefixTokens)}-token`,
              "minimum, so `cache_control` engages and a real interactive run bills",
              "well below this figure. Compare against the cached per-run number",
              "before treating this as a breaker decision.",
            ]),
        "",
        "Remaining levers, in order of preference:",
        "",
        `1. **Raise \`MAX_RUN_COST_USD\`** (currently ${usd4(cap)}). It is a spec`,
        "   13 §1 [DEFAULT], so an Abhinav decision, and it interacts with the",
        "   daily budget breaker: a higher per-run cap buys proportionally fewer",
        "   runs per day.",
        "2. **Cut turns.** Every model turn re-reads the whole prefix, so turn",
        "   count is what drives the cost. Parallel tool calls would collapse",
    ...
    "",
    "**F4. Thinking defaults differ across the matrix — settled.** Omitting",
    "the `thinking` parameter means no thinking on `claude-haiku-4-5` but",
    "adaptive thinking **on by default** for `claude-sonnet-5` and",
    "`claude-opus-5`, which would have compared a non-thinking Haiku against",
    "two thinking models. LOT-119 resolved it: the eval live lane sets",
    '`thinking: {type: "disabled"}` for matrix runs, and the agent loop',
    "forwards that only to models that accept it, so the production Haiku path",
    "is unchanged. The matrix now runs all three models on the same",
    "no-thinking basis.",
    "",
    "---",
    "",
    ...
    "`npm test` never contacts the API. A machine-readable `.json` is written",
    "alongside this file by the same run.",
    "",
