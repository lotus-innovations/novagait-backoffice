...
export async function resumeRun(
  ...
  const machineOrNull = await RunStateMachine.load(store, runId);
  if (!machineOrNull) throw new Error(`unknown run: ${runId}`);
  const machine = machineOrNull;
  if (machine.state.step !== "awaiting_approval") {
    throw new Error(`run ${runId} is not awaiting approval`);
  }
  const approvalOrNull = await getApprovalForRun(store, runId);
  if (approvalOrNull === null || approvalOrNull.status !== "pending") {
    throw new Error(`run ${runId} has no pending approval`);
  }
  const approval = approvalOrNull;
  ...
  const executionOrNull = readExecution(machine.state.data.execution);
  if (!executionOrNull) {
    throw new Error(`run ${runId} has no valid stashed execution`);
  }
  const execution = executionOrNull;
  ...
  const claimKey = `approval:claim:${approval.approval_id}`;
  const claims = await store.incrBy(claimKey, 1, CLAIM_TTL_SECONDS);
  if (claims > 1) {
    throw new Error(
  ...
  try {
    return await decideAndFinish();
  } catch (error) {
    // Reviewer-gate fix: a failure that left the approval PENDING must
    // release the claim, or the approval becomes undecidable for the
    // claim TTL - the same bricking the executor path already avoids.
    const current = await getApprovalForRun(store, runId);
    if (
  ...
  async function decideAndFinish(): Promise<ResumeResult> {
    ...
    if (input.decision === "reject") {
      ...
      if (machine.state.revision_count < MAX_REVISIONS) {
        ...
        await writer.append({
          ...
          result_summary: JSON.stringify({ draft_ref: revisedRef }).slice(
            0,
            160,
          ),
          duration_ms: 0,
